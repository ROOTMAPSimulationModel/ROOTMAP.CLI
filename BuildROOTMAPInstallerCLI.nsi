...
Name ROOTMAP
OutFile "InstallROOTMAPCLI-v${SEMVER}.exe"
ShowInstDetails "nevershow"
ShowUninstDetails "nevershow"
RequestExecutionLevel admin ; Admin is required to install VC++ redistributable, add ROOTMAP CLI to Start menu etc.
...
VIAddVersionKey ProductName      "ROOTMAP CLI"
...
VIAddVersionKey FileDescription  "ROOTMAP is a simulation for modelling root systems. This is the command line-only variant, for running simulations unattended."
VIAddVersionKey FileVersion      "${SEMVER}"
VIAddVersionKey ProductVersion   "${SEMVER}"
VIAddVersionKey InternalName     "ROOTMAP CLI"

;--------------------------------
; Folder selection page

InstallDir "$APPDATA\ROOTMAP-CLI\v${SEMVER}"
...
do_network_install:
    ...
    fail:
        MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to download Visual C++ Redistributable. Please try again. If still unsuccessful, you can try installing the Redistributable from https://aka.ms/vs/15/release/VC_redist.x86.exe separately, rebooting, then installing ROOTMAP CLI."
        Goto done
...
Section "install"
  ...
  CreateShortCut "$SMPROGRAMS\ROOTMAP\Uninstall ROOTMAP CLI v${SEMVER}.lnk" "$INSTDIR\application\Uninstall.exe" "" "$INSTDIR\application\Uninstall.exe" 0
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\App Paths\RootmapCLI.exe" "" "$APPDATA\ROOTMAP-CLI\v${SEMVER}\application\RootmapCLI.exe"

  ; write uninstall information to the registry
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "DisplayName" "ROOTMAP CLI (remove only)"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "UninstallString" "$INSTDIR\application\Uninstall.exe"
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "Publisher" "University of Western Australia"

  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "DisplayVersion" "${SEMVER}"
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "NoModify" "1"
  WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}" "NoRepair" "1"

  ; ${GetSize} "$INSTDIR" "/S=0K" $0 $1 $2
  ; IntFmt $0 "0x%08X" $0
  ; WriteRegDWORD HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-${SEMVER}" "EstimatedSize" "$0"
...
Section "Uninstall"
  ...
  MessageBox MB_OK "ROOTMAP CLI will be uninstalled. Configurations, Raytracing and Postprocessing directories will remain untouched. If you don't need any of these files, please manually delete the directory $APPDATA\ROOTMAP-CLI\v${SEMVER}."

  ; Delete application files
  RMDir /r "$APPDATA\ROOTMAP-CLI\v${SEMVER}\application\*.*"
  RMDir "ROOTMAP CLI"
  ; Delete tools
  RMDir /r "$APPDATA\ROOTMAP-CLI\v${SEMVER}\tools\*.*"
  RMDir "$APPDATA\ROOTMAP-CLI\v${SEMVER}\tools"
  ; Delete documentation files
  RMDir /r "$APPDATA\ROOTMAP-CLI\v${SEMVER}\Documentation\*.*"
  RMDir "$APPDATA\ROOTMAP-CLI\v${SEMVER}\Documentation"
  ...
  Delete "$SMPROGRAMS\ROOTMAP\Uninstall ROOTMAP CLI v${SEMVER}.lnk"
  ; TODO reenable Delete "$SMPROGRAMS\ROOTMAP\ROOTMAP Configuration App v${SEMVER}.lnk"
  RMDir  "$SMPROGRAMS\ROOTMAP"

  ; Delete Registry Entries
  DeleteRegKey HKEY_LOCAL_MACHINE "SOFTWARE\ROOTMAPCLI-v${SEMVER}"
  DeleteRegKey HKEY_LOCAL_MACHINE "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\ROOTMAPCLI-v${SEMVER}"
...
Function .onInstSuccess
  MessageBox MB_OK "ROOTMAP CLI v${SEMVER} installed successfully. Open a command prompt in $APPDATA\ROOTMAP-CLI\v${SEMVER}\application\ and run RootmapCLI.exe."
FunctionEnd

Function un.onUninstSuccess
  MessageBox MB_OK "ROOTMAP CLI v${SEMVER} uninstalled successfully."
FunctionEnd
